...
lemma grad_bernoulli_neglogpdf₂ (k : TReal → TReal) (shape : S) (p z : T shape)
  ...
  ∇ (λ z => k (bernoulli_neglogpdf p z)) z = ∇ k (bernoulli_neglogpdf p z) •  (log (eps shape + (1 - p)) - log (eps shape + p)) := by
  have H_diff₁ : is_cdifferentiable (λ (θ₀ : T shape) => k (-T.sum (θ₀ * T.log (eps shape + p) + (1 - z) * T.log (eps shape + (1 - p))))) z := by proveDifferentiable
  have H_diff₂ : is_cdifferentiable (λ (θ₀ : T shape) => k (-T.sum (z * T.log (eps shape + p) + (1 - θ₀) * T.log (eps shape + (1 - p))))) z := by proveDifferentiable
