...
lemma can_diff_under_ints_alt1 {costs : list ID}
  {ref : Reference} {parents : List Reference} {op : Operator parents.p2 ref.2} {nodes : List Node} {inputs : Env} {tgt : Reference} :
  let θ : T tgt.2 := env.get tgt inputs
  let g : T ref.2 → T tgt.2 → TReal :=
  (λ (x : T ref.2) (θ₀ : T tgt.2), by
...
lemma pdf_continuous {ref : reference} {parents : list reference} {op : rand.op parents^.p2 ref.2}
  ...
  ∀ (y : T ref.2),
    T.is_continuous (λ (x : T tgt.2),
                      (op^.pdf (dvec.update_at x (env.get_ks parents (env.insert tgt θ inputs)) idx) y))
                      (env.get tgt inputs) :=
begin
...
have H_ref_notin_parents : ref ∉ parents, from ref_notin_parents H_wf^.psInEnv H_wf^.uids,
...
have H_wfs : wellFormedAt costs nodes next_inputs tgt ∧ wellFormedAt costs nodes next_inputs ref, from wf_at_next H_wf,
...
have H_wfs : ∀ y, wellFormedAt costs nodes (next_inputs y) tgt ∧ wellFormedAt costs nodes (next_inputs y) ref,
...
lemma rest_continuous {costs : list ID} {n : Node} {nodes : List Node} {inputs : Env} {tgt : Reference} {x : T n.ref.2} :
  ...
  wellFormedAt costs nodes (env.insert n.ref x^.head inputs) tgt → grads_exist_at nodes (env.insert n.ref x^.head inputs) tgt →
  T.is_continuous (λ (θ₀ : T tgt.2),
                    E (graph.to_dist (λ (m : env), ⟦sum_costs m costs⟧)
...
lemma pd_is_cdifferentiable (costs : list ID) : Π (tgt : Reference) (inputs : Env) (nodes : List Node),
  ...
  T.is_cdifferentiable (λ (θ₀ : T tgt.snd), E (graph.to_dist (λ m, ⟦sum_costs m costs⟧) (env.insert tgt θ₀ inputs) nodes) dvec.head) (env.get tgt inputs)
