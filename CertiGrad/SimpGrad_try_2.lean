...
partial def SimpGradCoreLoop
    ...
    (lhs: Expr)
    : TacticM (Option (Expr × Expr × List MVarId × Expr)) := do
  ...
  --   -- logInfo m!"ManualSimpGradCore is invoked, lhs={lhs}, rhs={e}"
    ...
    if fn.isConstOf ``certigrad.T.grad && args.size == 3 then
        ...
        try
           ...
           logInfo m!"{ newGoal.mvarId!}"
          --  let symmproof ←  mkEqSymm proof

           let final: Expr := subresult.eNew
          ...
          logInfo m!"Catch error in SimpGradCoreLoop, ex={ex.toMessageData}"
          return some (lhs, lhs, [], ← mkEqRefl lhs)
    else
      try
        match lhs with
          | Expr.app f x =>
              try
                ...
                let (flhs, frhs, fsubgoals, fproof) ← (← SimpGradCoreLoop f)
                let (xlhs, xrhs, xsubgoals, xproof) ← (← SimpGradCoreLoop x)
                let newlhs := mkApp flhs xlhs
                let newrhs := mkApp frhs xrhs
                let subgoals := fsubgoals ++ xsubgoals
                let newproof ← mkCongr fproof xproof
                logInfo m!"Call after ---match lhs with---, newlhs={newlhs}, newrhs={newrhs}"
                return (some (newlhs, newrhs, subgoals, newproof))
              catch _ =>
                let proof ← mkEqRefl lhs
                return (some (lhs, lhs, [], proof))
          | _ =>
              let proof ← mkEqRefl lhs
              return (some (lhs, lhs, [], proof))
      catch ex =>
...
partial def SimpGradCore (tid: MVarId)  : TacticM ((List MVarId)) := do
  ...
  match e.eq? with
  | some (_, lhs, _) =>
    let (newlhs, newrhs, subgoals, proof) ← (←  SimpGradCoreLoop lhs)
